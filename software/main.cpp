...
void setComparatorVoltageLimit(int voltage) {
    Serial.print("Voltage to set: ");
    Serial.println(voltage);
    if (voltage < 100 || voltage > 1500) return;
...
void processInterrupt() {
    ...
    if (true) {  // currentInterrupt - lastInterrupt < 100 && currentInterrupt - lastInterrupt > 20
        ...
        if (pauseTime > 18000 && messageStarted) {
            ...
            Serial.println("No Start Message Detected");
        }
    ...
    switch (msgCode) {
        ...
        case MSG_CALL_FROM_ENTRY_DOOR_SCREEN_ON:
            if (ringToOpenStarted) {
                // Waiting time in necessary otherwise the opening signal cant get through
                delay(2000);
...
void function_ISR() {
    ...
    if (mySettings.filter) {
        if (interrupt_time - last_interrupt_time > 20 && interrupt_time - last_interrupt_time < 125)  // 4 times 40ms = 25kHz
        {
            if (!pinTriggered) {
                pinTriggered = true;
            }
...
void setup_wifi(bool autoconnect = true, unsigned long config_portal_timeout = 120) {
...
void callback(char *topic, byte *message, unsigned int length) {
    ...
    if (String(topic) == "SimpleBus/ActivateConfigPortal") {
        ...
        if (isNumeric(messageTemp)) {
            ...
            timeoutTime = max(min(timeoutTime, 1), 1440);
        }
        digitalWrite(LEDPIN, HIGH);
        setup_wifi(false, timeoutTime);
